...
from src.assistants.classic.orchestrator import ActionType
from src.assistants.classic.orchestrator import RoutingDecision
from src.assistants.classic.orchestrator import route_query
...
RESPONSE_PROMPT = '''You are a helpful company assistant for ZURU Melon, an AI solutions company.
...
class CompanyAssistant:
    ...
    def cache(self) -> QueryCache | None:
        ...
        if self._cache is None and self.use_cache:
            try:
                self._cache = get_cache(agent_type='classic')
    ...
    def _generate_response(self, query: str, context: str) -> str:
        ...
        for msg in self.conversation_history[-8:]:# maximum 8 messages in memory 
            messages.append(msg)
